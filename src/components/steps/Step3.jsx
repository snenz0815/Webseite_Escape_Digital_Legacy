...
const requiredFields = ['firstName', 'lastName', 'email', 'relationship', 'street', 'zip', 'city', 'state', 'country'];
...
const Step3 = ({ data, onUpdate }) => {
  ...
  const fieldClass = (name) => {
    ...
    return `w-full px-4 py-3 rounded-xl border focus:ring-2 outline-none transition-all ${
      isEmpty ? 'border-red-300 bg-red-50 focus:border-red-400 focus:ring-red-100' : 'border-gray-200 focus:border-primary-500 focus:ring-primary-200'
    }`;
  };
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-gray-900 mb-2">Ihre Kontaktdaten (Antragsteller)</h2>
      <p className="text-gray-500 mb-2">Bitte geben Sie Ihre eigenen Daten als Antragsteller an – also als Person, die den Antrag im Namen des Verstorbenen stellt.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <input type="text" name="firstName" value={data.firstName} onChange={handleChange}
            placeholder="Erika" className={fieldClass('firstName')} />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-700">E-Mail-Adresse <span className="text-red-500">*</span></label>
          <input type="email" name="email" value={data.email} onChange={handleChange}
            placeholder="erika@beispiel.de" className={fieldClass('email')} />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-700">Telefonnummer (optional)</label>
          <input type="tel" name="phone" value={data.phone} onChange={handleChange}
            placeholder="+49 123 4567890"
            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 outline-none transition-all" />
        </div>
        <div className="md:col-span-2 space-y-2">
          <label className="text-sm font-bold text-gray-700">Verhältnis zum Verstorbenen <span className="text-red-500">*</span></label>
          <select name="relationship" value={data.relationship} onChange={handleChange}
            className={fieldClass('relationship') + ' bg-white'}>
            <option value="">Bitte wählen...</option>
            <option value="Ehepartner / Lebenspartner">Ehepartner / Lebenspartner</option>
            <option value="Kind (volljährig)">Kind (volljährig)</option>
            ...
            <option value="Nachlasspfleger">Nachlasspfleger</option>
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <input type="text" name="street" value={data.street} onChange={handleChange}
            placeholder="Musterstraße 2" className={fieldClass('street')} />
        </div>
        <div className="space-y-2">
